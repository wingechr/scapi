...
def encode_content(data, schema):
    ...
    logging.debug("%s(%s) -> %s = %s(%s)", data, data.__class__.__name__, schema, result, result.__class__.__name__)
    return result
...
def from_string(data, type):
    if type == "integer":
        ...
        result = data
    logging.debug("%s(%s) -> %s = %s(%s)", data, data.__class__.__name__, type, result, result.__class__.__name__)
    return result
